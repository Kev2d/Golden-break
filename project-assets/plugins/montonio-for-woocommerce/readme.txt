...
* Dev – `wc_montonio_payment_reference` filter now receives the order ID as a third argument
...
= 10.1.2 =
* Fix – Telemetry data is now sent immediately after connecting the store via the Partner System

= 10.1.1 =
* Added – Resync Data button on the Main settings page to manually refresh payment methods, shipping methods and pickup points from Montonio
* Fix – Payment method sync is now re-enabled after Main settings are saved
...
* Dev – Converted callback handler to fully static methods with dedicated `handle_return()` and `handle_notification()` entry points
* Dev – Co-located class initialization calls with their respective file includes in the main plugin file
* Dev – Removed Montonio_Singleton base class
...
* Tweak – Sync pickup-points by carrier instead of country for better performance
* Fix – Users without certain admin permissions could not dismiss the info banner
